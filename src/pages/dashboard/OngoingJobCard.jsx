...
const OngoingJobCard = ({ job }) => {
  console.log("first", job);
  const { t } = useTranslation();
  return (
    <div className="bg-white rounded-2xl px-4 py-4 sm:px-5 sm:py-5 md:px-6 md:py-6 space-y-4 shadow hover:shadow-lg transition ">
      {/* Job Title */}
      ...
      {/* Job Details */}
      <div className="flex flex-wrap gap-2 sm:gap-3 mt-3 text-xs font-bold">
        <span className="bg-[#56CDAD1A] text-[#56CDAD] px-2 py-1 sm:px-1 sm:py-1 rounded-full ">
          {t("Start from")}: {job.validTill}
        </span>
        <span className="border border-[#FFB836] text-[#FFB836] px-2 py-1 sm:px-1 sm:py-1 rounded-full ">
          {job.time}
        </span>
        <span className="border border-black px-2 py-1 sm:px-3 sm:py-1 rounded-full">
          {job.pay}
        </span>
      </div>
    </div>
